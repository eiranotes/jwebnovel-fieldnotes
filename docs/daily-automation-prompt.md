...
13. Use `python3 scripts/translate_project.py --max-chunks N` as the sole translation driver. Primary translation uses one exact ChatGPT Project conversation per `(work_id, role)`, fresh Project-source proof, exact local task reads, strict sentence/glossary validation, and transactional completion. Never manually translate/complete a queue task outside this driver.
14. The driver is fail-closed across both model lanes. Primary is `webgpt_project`; configured fallback is `codex_webgpt`, where local Codex prepares the instruction and Oracle drives WebGPT from a throwaway copy of the signed-in Chrome profile. Fallback is permitted only for the explicit `fallback.trigger_codes` in `config/project-translation.json`. `WORKER_RESULT_TIMEOUT`, uncertain submission/transport, source/hash changes, and commit uncertainty must stop without fallback or resubmission.
